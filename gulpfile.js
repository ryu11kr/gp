...
var 
	...
	gulpif = require('gulp-if'), //파일명을 새롭게 정의하는 플러그인 구동
...
gulp.task('watch', function(){
	watch(config.htmlset.src, function() { //HTML 템플릿 업무 관찰
		gulp.start('html'); //html task 호출
	});
	watch(config.scssset.src, function() { //scss 업무 관찰
		gulp.start('scss'); //scss task 호출
	});
	watch(config.cssset.src, function() { //css 업무 관찰
	...
	watch(config.jsset.src, function() { //css 업무 관찰
		gulp.start('js'); //css task 호출
	});
});

// 웹 서버
gulp.task('connect', connect.server( config.serverset )); // config.js 의 server 설정 호출

// HTML 변경 내용, 자동 갱신(업데이트)
gulp.task('html', function() {
	gulp
		.src(config.htmlset.src) // html 타겟 설정
		.pipe(connect.reload()) // 타겟 html 자동 리로드
});
// CSS 변경 내용, 자동 갱신(업데이트)
gulp.task('css', function() {
	gulp
		.src(config.cssset.src) // css 타겟 설정
		.pipe(connect.reload()) // 타겟 css 자동 리로드
		//min.css 만들경우
		.pipe(gulpif(config.cssset.uglify, uglifycss())) //css 코드 압축
		.pipe(gulpif(config.cssset.rename, rename({suffix:'.min'}))) //압축된 css 파일명에 .min 붙임
		.pipe(gulp.dest(config.cssset.minsrc)); //압축된 css 파일이 생성될 위치 지정
});
...
gulp.task('js', function() {
	gulp
		.src(config.jsset.src) // js 타겟 설정
		.pipe(connect.reload()) // 타겟 js 자동 리로드
		//min.js 만들경우
		.pipe(gulpif(config.jsset.uglify, uglify()))
		.pipe(gulpif(config.jsset.rename, rename({suffix:'.min'})))
		.pipe(gulp.dest(config.jsset.minsrc));
});
// SCSS 변경 내용, 자동 갱신(업데이트)
gulp.task('scss', function() {
	gulp
	.src(config.scssset.src) // scss 타겟  설정
	...
	.pipe(compass({ // 사스 프레임워크 컴파스 호출
		css: config.scssset.target, // scss 에서 css 로 변환될 위치 지정
		sass: config.scssset.compasssrc, //컴파스 동작할 scss 경로 지정
		style: 'expanded' // css 아웃풋 설정, nested, expanded, compact, compressed
	}))
	.pipe( gulp.dest(config.scssset.target) ); //css로 변환된 코드를 보낼 css 디렉토리 지정
});
